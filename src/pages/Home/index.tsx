  ...
  const columns:ColumnsType<TWeather> = [
    ...
    },
    {
      ...
      render:(value, record, index)=>(
        <span>{record.condition.text}</span>
      )
    },
  ]
  const fetch = () => {
        ...
        <Tabs.TabPane tab={homeTxts.tabs.cardTab[language]} key="1">
          <div className="flex flex-row flex-wrap justify-start items-start" >
            {weathers.map((weather, index) => {
              return <MyCard weather={weather} key={index} />;
            })}
          </div>
        </Tabs.TabPane>
        <Tabs.TabPane tab={homeTxts.tabs.tableTab[language]} key="2">
          <Table className="hover:cursor-pointer" onRow={(record,index)=>{return{
            onClick:()=>{setWeather(record);setDrawerShow(true)}
          }}}  onChange={(page:TablePaginationConfig)=>setPagination(page)} pagination={pagination} columns={columns} dataSource={weathers} />
        </Tabs.TabPane>
        {drawerShow && (
        <WeatherDrawer
          visible={drawerShow}
